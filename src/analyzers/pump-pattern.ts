...
export interface PriceDataPoint {
  ...
  volume: number; // Trading volume in USDT
}
...
export interface PumpPatternAnalysis {
  score: number; // 0-100
  priceMultiplier: number; // Current price / launch price
  volumeMultiplier: number; // Current volume / average volume
  volumeTrend: 'increasing' | 'decreasing' | 'stable';
  priceMomentum: number; // Rate of price change
  volatility: number; // Standard deviation of returns
  hasExecutionRisk: boolean; // Detected pump-and-dump indicators
  riskFlags: string[];
  positives: string[];
}
...
export class PumpPatternAnalyzer {
  ...
  analyzePumpPattern(priceHistory: PriceDataPoint[], launchPrice: number): PumpPatternAnalysis {
    ...
    if (!Array.isArray(priceHistory) || priceHistory.length === 0) {
      logger.warn('Empty price history provided for pump pattern analysis');
      return this.createNeutralAnalysis('No price history available');
    }

    if (launchPrice <= 0) {
      throw new APIError('Invalid launch price', { launchPrice });
    }
    ...
    try {
      ...
      throw new APIError('Failed to analyze pump pattern', {
   ...
   */
  private analyzeVolumeTrend(sortedHistory: PriceDataPoint[]): 'increasing' | 'decreasing' | 'stable' {
   ...
   */
  private calculatePriceMomentum(sortedHistory: PriceDataPoint[]): number {
    if (sortedHistory.length < 2) {
      return 0;
    }
   ...
   */
  private calculateVolatility(sortedHistory: PriceDataPoint[]): number {
    if (sortedHistory.length < 2) {
      return 0;
    }
   ...
   */
  private detectExecutionRisk(sortedHistory: PriceDataPoint[], launchPrice: number): boolean {
   ...
   */
  private createNeutralAnalysis(reason: string): PumpPatternAnalysis {
    return {
      score: 50,
      priceMultiplier: 1,
      volumeMultiplier: 1,
