...
class ManualTestHelper {
    ...
    async verifyTokenDetection() {
        ...
        try {
            ...
            this.addResult('Phase 2: Token Detection', true, `${count} tokens detected and processed`, Date.now() - start);
        }
        catch (error) {
    ...
    async verifyTelegramAlerts() {
        ...
        try {
            ...
            this.addResult('Phase 3: Telegram Alerts', true, `${alerts.length} alerts sent successfully`, Date.now() - start);
        }
        catch (error) {
    ...
    async verifyScoring() {
        ...
        try {
            ...
            this.addResult('Phase 4: Scoring Accuracy', true, `Avg score: ${avgScore.toFixed(1)}/100 | Safe: ${distribution.safe}, Caution: ${distribution.caution}, Avoid: ${distribution.avoid}`, Date.now() - start);
        }
        catch (error) {
    ...
    async verifyFalsePositiveRate() {
        ...
        try {
            ...
            this.addResult('Phase 5: False Positive Rate', pass, `Alert rate: ${(100 - falsePositiveRate).toFixed(1)}% | Processed: ${processed}, Alerted: ${alerted}`, Date.now() - start);
        }
        catch (error) {
    ...
    async verifyErrorRecovery() {
        ...
        try {
            ...
            this.addResult('Phase 6: Error Recovery', pass, `Errors: ${errors.length}, Recovery attempts: ${recovered.length}`, Date.now() - start);
        }
        catch (error) {
    ...
    async verifyDatabasePersistence() {
        ...
        try {
            ...
            if (!fs_1.default.existsSync('data/bot.db')) {
                throw new Error('Database file not created. Bot may not have initialized.');
            }
            this.addResult('Phase 7: Database Persistence', true, 'Database file exists and accessible', Date.now() - start);
        }
        catch (error) {
    ...
    async verifyPerformance() {
        ...
        try {
            ...
            this.addResult('Phase 8: Performance', pass, `Avg: ${avgLatency.toFixed(2)}s, Max: ${maxLatency.toFixed(2)}s (target: <3s)`, Date.now() - start);
        }
        catch (error) {
     ...
     */
    addResult(phase, passed, message, duration) {
     ...
     */
    getTempLogFile() {
        return path_1.default.join(process.cwd(), '.test-logs.txt');
    }
    /**
...
exports.ManualTestHelper = ManualTestHelper;
